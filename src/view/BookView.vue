...
export default {
    ...
    created(){
        ...
        //             this.count.push(parseInt(item.position))
        //         });
    ...
    async mounted() {
        await axios.get('http://localhost/data/api/bill/filter.php?status=1&method=1')
            .then(data=>{
                data.data.forEach(item=>{
        ...
        this.count.forEach(item => {           
            ...
            text[item-1].addEventListener('click',()=> {
                ...
                axios.get(`http://localhost/data/api/detailBill/selectByPosition.php?position=${item}`)
                .then(data=>{
